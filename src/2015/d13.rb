...
ppl={}
input.lines.each{ |l|
  mat=l.match(/^(\w+) would (gain|lose) (\d+) happiness units by sitting next to (\w+).$/)
  nbv[[mat[1],mat[4]].sort]+=(mat[2]=="gain" ? 1 : -1)*mat[3].to_i
  ppl[mat[1]]=true
}
max=ppl.keys.permutation.map{ |perm|
  perm.cycle.take(ppl.length+1).each_cons(2).map{ |a,b|
    nbv[[a,b].sort]
  }.reduce(:+)
}.max
p max
max2=ppl.keys.permutation.map{ |perm|
  perm.each_cons(2).map{ |a,b|
    nbv[[a,b].sort]
  }.reduce(:+)
}.max
p max2
