...
input=Input[DATA].r
leds=Array::new(1000){Array::new(1000,0)}
funcs={
  "toggle"=>->(v){v+2},
  "turn on"=>->(v){v+1},
  "turn off"=>->(v){[0,v-1].max},
}
input.lines.each{ |l|
  mat=l.match(/^(toggle|turn on|turn off) (\d+),(\d+) through (\d+),(\d+)/)
  f=funcs[mat[1]]
  x1,y1,x2,y2=mat[2,4].map(&:to_i)
  x1,x2=[x1,x2].sort
  y1,y2=[y1,y2].sort
  (x1..x2).each{ |x|
    (y1..y2).each{ |y|
