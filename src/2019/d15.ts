...
class Board {
  ...
  set([x, y]: Pos, item: Field) {
    const r = this.board[y] || [];
    this.board[y] = r;
    r[x] = item;
  }
  ...
  getAll() {
    const width = Math.max(0, ...this.board.map(r => r.length).filter(l => l));
    return this.board.filter(r => r).map(rr => {
      const r: Field[] = [];
      r.length = width;
      r.fill(" ");
      rr.forEach((i, x) => {
...
const dirDeltas = [[], [0, -1], [0, 1], [-1, 0], [1, 0]];

async function run1() {
  const board = new Board();
  const start: Pos = [50, 50];
  let p: Pos = [start[0], start[1]];
  let oxy: Pos | undefined;
  board.set(p, ".");
  function nPos(dir: number): Pos {
    const dirDelta = dirDeltas[dir];
  ...
  async function explore() {
    ...
    for (let dir = 1; dir <= 4; dir++) {
      if (board.get(nPos(dir)) === " ") {
        const out = await step(dir);
        if (out !== 0) {
          if (out === 2)
            foundOxy = true;
          if (await explore())
...
async function run2() {
  function nPos(p: Pos, dir: number): Pos {
    const dirDelta = dirDeltas[dir];
    return [p[0] + dirDelta[0], p[1] + dirDelta[1]];
  }
  ...
  while (oxyPoss.length) {
    const newOxyPoss = await oxyStep();
