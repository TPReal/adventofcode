...
const VALIDATION: Record<string, (v: string) => unknown> = {
  byr: v => v.match(/^\d{4}$/) && Number(v) >= 1920 && Number(v) <= 2002,
  iyr: v => v.match(/^\d{4}$/) && Number(v) >= 2010 && Number(v) <= 2020,
  eyr: v => v.match(/^\d{4}$/) && Number(v) >= 2020 && Number(v) <= 2030,
  hgt: v => {
    let mat = v.match(/^(\d{3})cm$/);
    if (mat && Number(mat[1]) >= 150 && Number(mat[1]) <= 193)
      return true;
    mat = v.match(/^(\d{2})in$/);
    if (mat && Number(mat[1]) >= 59 && Number(mat[1]) <= 76)
      return true;
    return false;
  },
...
export function run() {
  ...
  for (const doc of INPUT.split("\n\n")) {
    const data = new Map(doc.trim().split(/\s+/).map(p => p.trim().split(":") as [string, string]));
    if (Object.keys(VALIDATION).every(k => k === "cid" || data.has(k)) &&
      [...data.entries()].every(([k, v]) => {
