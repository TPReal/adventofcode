...
export function run() {
  ...
  for (const l of INPUTS[0].trim().split("\n")) {
    const mat = l.match(/^(\w+ \w+) bags contain (.+)\./);
    if (!mat)
      throw new Error(`Bad line: ${l}`);
    const list: [number, string][] = [];
    rules.set(mat[1], list);
    if (mat[2] !== "no other bags")
      for (const b of mat[2].split(", ")) {
        const mat = b.match(/^(\d+) (\w+ \w+) bags?$/);
        if (!mat)
          throw new Error(`Bad bags desc: ${b}`);
        list.push([Number(mat[1]), mat[2]]);
      }
  }
  const myBag = "shiny gold";
  const hasMy = new Map<string, boolean>();
  function getHasMy(b: string) {
    if (hasMy.has(b))
      return hasMy.get(b);
    if (b === myBag) {
      hasMy.set(b, true);
    ...
    const list = rules.get(b);
    if (!list)
      throw new Error(`No bag rules: ${b}`);
    const res = list.some(([_, b2]) => getHasMy(b2));
    hasMy.set(b, res);
    return res;
  }
  for (const b of rules.keys()) {
    getHasMy(b);
  }
  log([...hasMy.values()].filter(Boolean).length - 1);
  function cnt(b: string): number {
    const list = rules.get(b);
    if (!list)
      throw new Error(`No bag rules: ${b}`);
    return list.map(([c, b2]) => c * cnt(b2)).reduce((a, b) => a + b, 1);
  }
  log(cnt(myBag) - 1);
}
